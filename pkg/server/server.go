...
func (server *BLEServer) Run() error {
	...
	for addr := range server.clientStateMap {
		server.setClientState(addr, BLEClientState{Name: "", Status: Disconnected, ConnectedAddr: "", RssiMap: RssiMapData{}})
	}
	return err
}
		...
		for {
			...
			for addr := range lastHeard {
				if diff > lastHeard[addr] {
					state := BLEClientState{Name: "", Status: Disconnected, ConnectedAddr: "", RssiMap: RssiMapData{}}
